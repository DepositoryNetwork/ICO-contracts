...
contract('ReserveTokenVesting', function (accounts) {
    ...
    const thirdPeriod = secondPeriod + interval;
    const fourthPeriod = thirdPeriod + interval;
    ...
    describe("Set values correctly", () => {
        ...
        it("should be owned by owner", async function () {
            let _owner = await vestingContract.reserveWallet({
                from: contractOwnerAddress
            });
        ...
        it("should set reserve wallet correctly", async function () {
            let result = await vestingContract.reserveWallet.call();
    ...
    describe("Claim withdraws", () => {
        ...
        it("should transfer the tokens to the owner for the first period", async function () {
            ...
            assert(initialOwnerBalance.add(amount * (25/100)).eq(finalOwnerBalance), "Claim amount was not correct")
        });
        ...
        it("should transfer the tokens to the owner for the third period", async function () {
            ...
            await timeTravel(web3, thirdPeriod);
            await vestingContract.claim({
                from: wallet
            });
            ...
            assert(initialOwnerBalance.add(amount * (75/100)).eq(finalOwnerBalance), "Claim amount was not correct")
        });

        it("should transfer the tokens to the owner for the fourth period", async function () {
            let initialOwnerBalance = await tokenInstance.balanceOf(wallet);

            await timeTravel(web3, fourthPeriod);
            await vestingContract.claim({
                from: wallet
            });
